...
bool ImprovedViewer::key_press_event(int key, int modifiers) {
    ...
    else if (key == GLFW_KEY_T)
    {
        ...
        for(int i=0; i<n; i++)
        {
            z[i] = click_points[i].x*click_points[i].x + click_points[i].y*click_points[i].y;
            vertices[i] = camera()->unprojectedCoordinatesOf(click_points[i]);
        }
        ...
        for(int i=0; i<n-2; i++)
        {
            for(int j=i+1; j<n; j++)
            {
                for(int k=i+1; k<n; k++)
                {
                    if(j != k)
                    {
                        double u = (click_points[j].y - click_points[i].y)*(z[k]-z[i]) - (click_points[k].y - click_points[i].y)*(z[j]-z[i]);
                        double v = (click_points[k].x - click_points[i].x)*(z[j]-z[i]) - (click_points[j].x - click_points[i].x)*(z[k]-z[i]);
                        double w = (click_points[j].x - click_points[i].x)*(click_points[k].y - click_points[i].y) - (click_points[j].y - click_points[i].y)*(click_points[k].x - click_points[i].x);
                        bool f = w<0;
                        int m = 0;
                        while(m < n && f)
                        {
                            if(((click_points[m].x - click_points[i].x)*u + (click_points[m].y - click_points[i].y)*v + (z[m]-z[i])*w) > 0)
                                f = false;
                            m++;
                        }
                        if(f)
        ...
        last_one = new LinesDrawable("triangulation");
        last_one->update_index_buffer(indices);
        last_one->update_vertex_buffer(vertices);
        last_one->set_default_color(vec3(1,0,1));
